...
class TrainDDP:
    def __init__(self, args):
        ...
        self.coef_kdloss = args.coef_kdloss  # Reused as coefficient for MMD loss
    ...
    def define_loss(self):
        ...
        self.mmd_loss = MMDLoss(sigma=0.1).cuda()  # Replaced kd_loss with mmd_loss
    ...
    def train(self):
        ...
        if self.rank == 0:
            ...
            meter_mmdloss = meter.AverageMeter("MMDLoss", ":.4e")  # Changed from KDLoss to MMDLoss
        ...
        for epoch in range(self.start_epoch + 1, self.num_epochs + 1):
            ...
            with tqdm(total=len(self.train_loader), ncols=100, disable=self.rank != 0) as _tqdm:
                ...
                for images, targets in self.train_loader:
                    ...
                    with autocast():
                        ...
                        # Compute softened probabilities for MMD
                        p_teacher = torch.sigmoid(logits_teacher / self.target_temperature)
                        p_student = torch.sigmoid(logits_student / self.target_temperature)
                        mmd_loss = self.mmd_loss(p_teacher, p_student)  # Replaced kd_loss with mmd_loss
                        ...
                        total_loss = (
                            ori_loss
                            + self.coef_kdloss * mmd_loss  # Use mmd_loss with existing coefficient
                    ...
                    reduced_mmd_loss = self.reduce_tensor(mmd_loss)  # Updated to mmd_loss
                    ...
                    if self.rank == 0:
                        ...
                        meter_mmdloss.update(self.coef_kdloss * reduced_mmd_loss.item(), n)  # Updated to mmd_loss
                        meter_rcloss.update(
            ...
            if self.rank == 0:
                ...
                self.writer.add_scalar("train/loss/mmd_loss", meter_mmdloss.avg, global_step=epoch)  # Updated to mmd_loss
                ...
                self.logger.info(
                    ...
                    "LR {lr:.6f} "
                    "OriLoss {ori_loss:.4f} "
                    "MMDLoss {mmd_loss:.4f} "  # Updated to MMDLoss
                    "RCLoss {rc_loss:.4f} "
                    ...
                    "Train_Acc {train_acc:.2f}".format(
                        ...
                        mmd_loss=meter_mmdloss.avg,  # Updated to mmd_loss
